...
const FRAMES_CLOSED_EYES = {
  mouthClosed: NF_CC_CENTER,  // 깜빡임 시 방향은 정면으로 통일(원본 제공 프레임 기준)
...
// 카메라 + 가장 큰 얼굴 추적 → 좌/우 판정
async function startCameraAndTracking(){
  try{
    videoEl = document.createElement('video');
    videoEl.autoplay = true; videoEl.playsInline = true; videoEl.muted = true;
    Object.assign(videoEl.style, { position:'fixed', opacity:'0', pointerEvents:'none', width:'1px', height:'1px' });
      ...
      try{
        if (faceDetector && videoEl.readyState >= 2){
          ...
          if (faces && faces.length){
            ...
            eyeDir = -nx; // 셀카 반전 느낌: 좌우 반전
          } else {
            eyeDir *= 0.9; // 점진적으로 정면 복귀
          }
          showPortrait();
        }
